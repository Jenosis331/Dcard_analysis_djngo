...
def save_raw_articles_csv(df, keyword):
    ...
    output_dir = os.path.join("data", "dcard_data", "by_keyword")
...
def save_preprocessed_csv(df, keyword):
    ...
    safe_date_string = today.strftime("%Y%m%d")
    safe_keyword = keyword.replace(" ", "_")
    output_dir = os.path.join("data", "dcard_data", 'by_keyword', "dcard_data2_process")
    os.makedirs(output_dir, exist_ok=True)
...
def save_sub_preprocessed_csv(df, keyword):
    ...
    safe_date_string = today.strftime("%Y%m%d")
    safe_keyword = keyword.replace(" ", "_")
    output_dir = os.path.join("data", "dcard_data", 'by_keyword', "dcard_data2_sub_process")
    os.makedirs(output_dir, exist_ok=True)
...
def save_category_tokenpos_csv(df_articles, df_sub, keyword):
    ...
    safe_date_string = today.strftime("%Y%m%d")
    safe_keyword = keyword.replace(" ", "_")
    output_dir = os.path.join("data", "dcard_data", 'by_keyword', "dcard_data3_categray_topkey")
    os.makedirs(output_dir, exist_ok=True)
...
def main():
    ...
    max_articles = 20  # 設定最多抓取篇數，改為 None 表示不限制 
    ...
    try:
        ...
        try:
            ...
            log_path = os.path.join(log_dir, "crawler_error.log")
            with open(log_path, "a", encoding="utf-8") as f:
                f.write(f"=== CRAWLER CRASH: {today_str} ===\n")
