...
# 總資料表
output_path = os.path.join(BASE_DIR, "data", "csv_file", "dcard_data1_all.csv")
...
# 已修正原本代碼中的 len(new_df+) 語法錯誤
new_count = len(new_df)

# 2. 連接 SQLite 資料表並比對去重
...
try:
    cursor = conn.cursor()
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS dcard_data1_all (
    ...
    # 讀取現有資料庫中的 ID
    existing_db_ids = pd.read_sql_query("SELECT article_id FROM dcard_data1_all", conn)
    ...
    if add_count > 0:
        # 寫入 SQLite
        insert_df.to_sql("dcard_data1_all", conn, if_exists="append", index=False)
        print(f"SQLite 資料庫：已同步新增 {add_count} 筆資料")
    else:
        print("SQLite 資料庫：無新增資料 (已同步)")
