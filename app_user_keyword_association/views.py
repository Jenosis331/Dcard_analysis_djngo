...
def load_df_data_v1():
    ...
    csv_path = os.path.join(project_root, 'app_user_keyword', 'dataset', 'cna_news_preprocessed_all.csv')
...
def home(request):
    return render(request, 'app_user_keyword_association/home.html', {'categories': CATEGORIES})
...
def filter_dataFrame_fullText(user_keywords, cond, cate, weeks):
    ...
    if (cond == 'and'):
        ...
        condition = condition & df.content.apply(lambda text: any((qk in str(text)) for qk in user_keywords)) #寫法:any()
